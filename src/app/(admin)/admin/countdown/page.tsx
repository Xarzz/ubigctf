...
import { Play, Pause, RotateCcw, Maximize, Minimize, Settings2, TerminalSquare, ShieldAlert } from "lucide-react";
...
export default function LKSCountdownPage() {
    ...
    return (
        <div
            ref={containerRef}
            className={`w-full flex flex-col items-center justify-center transition-all duration-700 bg-[#050505] 
                ${isFullscreen ? 'fixed inset-0 z-50 p-8' : 'relative h-[calc(100vh-100px)] rounded-2xl border border-primary/20 shadow-[0_0_30px_rgba(239,68,68,0.1)] overflow-hidden'}`}
        >
            ...
            <div className="absolute top-8 left-8 flex items-center gap-3">
                ...
                <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-primary font-mono uppercase tracking-widest leading-none">
                        <HackerText text="CybSec Operation Timer" speed={30} delay={500} />
                    </span>
                </div>
            </div>
            ...
            {/* Configuration Panel */}
            {isConfiguring && (
                <div className="absolute inset-x-0 top-32 mx-auto max-w-lg p-6 bg-black/80 backdrop-blur-xl border border-primary/40 rounded-2xl shadow-[0_0_50px_rgba(239,68,68,0.15)] z-30 animate-in fade-in zoom-in-95 slide-in-from-top-4 duration-300">
            ...
            <div className="relative flex items-center justify-center scale-90 md:scale-100 transition-transform duration-500">
                {/* SVG Progress Circle */}
                <svg className="w-[420px] h-[420px] -rotate-90 drop-shadow-[0_0_25px_rgba(239,68,68,0.3)]">
                    <circle
                        cx="210"
                        cy="210"
                        r={radius}
                ...
                <div className="absolute flex flex-col items-center justify-center">
                    <div className="text-[5.5rem] font-bold font-mono tracking-tighter text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.4)] leading-none flex items-center select-none">
                        <span className="w-[110px] text-center tabular-nums">{h}</span>
                        <span className={`text-primary/70 pb-3 ${isRunning ? 'animate-pulse' : ''}`}>:</span>
                        <span className="w-[110px] text-center tabular-nums">{m}</span>
                        <span className={`text-primary/70 pb-3 ${isRunning ? 'animate-pulse' : ''}`}>:</span>
                        <span className="w-[110px] text-center tabular-nums text-primary">{s}</span>
                    </div>
