...
export function NavBar() {
    ...
    const { isActive, isInLKS, roomCode } = useLKSSession();
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [isChallengesDropdownOpen, setIsChallengesDropdownOpen] = useState(false);
    const [showLKSBlocked, setShowLKSBlocked] = useState(false);
    const [blockedAction, setBlockedAction] = useState("");

    const handleBlockedAction = (actionLabel: string) => {
        setBlockedAction(actionLabel);
        setShowLKSBlocked(true);
    };
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        <button className="flex items-center w-full text-left">
                                                            ...
                                                            <span>Log Out</span>
                                                            {isActive && <span className="ml-auto text-[9px] font-mono text-red-400/60 uppercase tracking-widest">Locked</span>}
                                                        </button>
                                                    </DropdownMenuItem>
                                                </>
                                            ) : (
